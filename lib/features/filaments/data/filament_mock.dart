import 'package:filament_nexus/features/filaments/domain/filament.dart';
import 'package:filament_nexus/features/filaments/domain/filament_detail.dart';
import 'package:filament_nexus/features/filaments/domain/filament_type.dart';
import 'package:filament_nexus/features/filaments/domain/filament_vendor.dart';
import 'package:filament_nexus/features/filaments/domain/filament_rating.dart';

final List<Filament> mockFilaments = [
  Filament(
    id: 'f-pla-basic',
    userId: 'edd369eb-4ed3-4681-ab41-775b2c2df76b',
    type: FilamentType(name: 'PLA'),
    vendor: FilamentVendor(name: 'Bambulab'),
    name: 'Basic',
    description: 'Vielseitiger Allrounder, einfach zu drucken, gut für Einsteiger.',
    printTempMin: 195,
    printTempMax: 215,
    bedTempMin: 30,
    bedTempMax: 50,
    printSpeedMin: 45,
    printSpeedMax: 150,
    fanSpeedFirstLayer: 0,
    fanSpeed: 100,
    details: FilamentDetails(
      ecoFriendly: true,
      foodSafe: true,
      forLightweightBuild: true,
    ),
    ratings: FilamentRatings(
      workability: 9,
      warping: 9,
      stringing: 10,
      shrinking: 10,
      draftSensitivity: 10,
    ),
  ),
  Filament(
    id: 'f-petg-iron',
    type: FilamentType(name: 'PETG'),
    vendor: FilamentVendor(name: 'Bambulab'),
    name: 'Iron',
    description: 'Robuster als PLA, leicht flexibel, gut für funktionale Teile.',
    printTempMin: 220,
    printTempMax: 240,
    bedTempMin: 40,
    bedTempMax: 60,
    printSpeedMin: 45,
    printSpeedMax: 150,
    fanSpeedFirstLayer: 80,
    fanSpeed: 80,
    details: FilamentDetails(
      uvResistant: true,
      solventResistant: true,
      abrasionResistant: true,
    ),
  ),
  Filament(
    id: 'f-abs-pro',
    type: FilamentType(name: 'ABS'),
    vendor: FilamentVendor(name: 'Prusa'),
    name: 'Pro',
    description: 'Temperaturbeständig, anspruchsvoll im Druck, gehäuse-pflichtig.',
    printTempMin: 235,
    printTempMax: 255,
    bedTempMin: 90,
    bedTempMax: 110,
    printSpeedMin: 35,
    printSpeedMax: 120,
    fanSpeedFirstLayer: 0,
    fanSpeed: 30,
    details: FilamentDetails(fireRetardant: true, solventResistant: true),
    ratings: FilamentRatings(
      workability: 7,
      warping: 4,
      stringing: 5,
      shrinking: 3,
      draftSensitivity: 6,
    ),
  ),
  Filament(
    id: 'f-tpu-flex',
    type: FilamentType(name: 'TPU'),
    vendor: FilamentVendor(name: 'Overture'),
    name: 'Flex',
    description: 'Gummiartig flexibel, ideal für Dichtungen und stossdämpfende Teile.',
    printTempMin: 210,
    printTempMax: 230,
    bedTempMin: 30,
    bedTempMax: 45,
    printSpeedMin: 20,
    printSpeedMax: 60,
    fanSpeedFirstLayer: 0,
    fanSpeed: 60,
    details: FilamentDetails(
      flexible: true,
      abrasionResistant: true,
      waterSoluble: false,
    ),
    ratings: FilamentRatings(
      workability: 6,
      warping: 5,
      stringing: 4,
      shrinking: 3,
      draftSensitivity: 2,
    ),
  ),
  Filament(
    id: 'f-asa-core',
    type: FilamentType(name: 'ASA'),
    vendor: FilamentVendor(name: 'Polymaker'),
    name: 'Core',
    description: 'UV-beständig und wetterfest, gute Wahl für Outdoor-Anwendungen.',
    printTempMin: 240,
    printTempMax: 260,
    bedTempMin: 90,
    bedTempMax: 110,
    printSpeedMin: 35,
    printSpeedMax: 120,
    fanSpeedFirstLayer: 0,
    fanSpeed: 20,
    details: FilamentDetails(uvResistant: true, ecoFriendly: false),
    ratings: FilamentRatings(
      workability: 5,
      warping: 6,
      stringing: 4,
      shrinking: 5,
      draftSensitivity: 7,
    ),
  ),
  Filament(
    id: 'f-pla-matte',
    userId: 'edd369eb-4ed3-4681-ab41-775b2c2df76b',
    type: FilamentType(name: 'PLA'),
    vendor: FilamentVendor(name: 'eSUN'),
    name: 'Matte',
    description: 'Matte Oberfläche ohne Glanz, gut für dekorative Modelle und Figuren.',
    printTempMin: 190,
    printTempMax: 210,
    bedTempMin: 30,
    bedTempMax: 50,
    printSpeedMin: 40,
    printSpeedMax: 120,
    fanSpeedFirstLayer: 0,
    fanSpeed: 100,
    ratings: FilamentRatings(
      workability: 7,
      warping: 3,
      stringing: 4,
      shrinking: 2,
      draftSensitivity: 5,
    ),
  ),
  Filament(
    id: 'f-pc-tough',
    type: FilamentType(name: 'PC'),
    vendor: FilamentVendor(name: 'Polymaker'),
    name: 'Tough',
    description: 'Extrem stabil und hitzefest, benötigt hohe Temperaturen und Erfahrung.',
    printTempMin: 260,
    printTempMax: 290,
    bedTempMin: 100,
    bedTempMax: 120,
    printSpeedMin: 20,
    printSpeedMax: 80,
    fanSpeedFirstLayer: 0,
    fanSpeed: 20,
    details: FilamentDetails(electricallyConductive: true, fireRetardant: true),
    ratings: FilamentRatings(
      workability: 4,
      warping: 7,
      stringing: 5,
      shrinking: 6,
      draftSensitivity: 8,
    ),
  ),
  Filament(
    id: 'f-pa-cf',
    type: FilamentType(name: 'PA-CF'),
    vendor: FilamentVendor(name: 'Bambulab'),
    name: 'Nylon Carbon',
    description: 'Carbonverstärkt und steif, ideal für leichte und belastbare Bauteile.',
    printTempMin: 250,
    printTempMax: 280,
    bedTempMin: 70,
    bedTempMax: 90,
    printSpeedMin: 30,
    printSpeedMax: 100,
    fanSpeedFirstLayer: 0,
    fanSpeed: 30,
    details: FilamentDetails(
      abrasionResistant: true,
      forLightweightBuild: true,
    ),
  ),
];
